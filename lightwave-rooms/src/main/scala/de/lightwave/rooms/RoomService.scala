...
import de.lightwave.rooms.RoomService.{GetRoom, PutInCache, RequestRoom}
...
class RoomService(roomRepository: RoomRepository) extends Actor {
  ...
  override def receive = {
    case GetRoom(id) => replicator ! Get(DataKey, ReadLocal, Some(RequestRoom(id, sender())))
    case PutInCache(id, room) => replicator ! Update(DataKey, LWWMap.empty[Room], WriteLocal)(_ + (id.toString -> room))

    case g @ GetSuccess(_, Some(RequestRoom(roomId, replyTo))) => g.dataValue match {
    ...
    case NotFound(_, Some(RequestRoom(roomId, replyTo))) => getRoomById(roomId) pipeTo replyTo
  }
}

object RoomService {
  case class GetRoom(id: RoomId)
  case class RequestRoom(id: RoomId, replyTo: ActorRef)
